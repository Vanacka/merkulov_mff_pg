...
while current_line != ["."]:
    if is_valid(current_line):
        ...
        seznam_mesicu.append(current_line)
    current_line = input().split()

mesic = seznam_mesicu[0][0].split(".")[1]
rok = seznam_mesicu[0][0].split(".")[2]
for i in range(len(seznam_mesicu)):
    if mesic == seznam_mesicu[i][0].split(".")[1]:
        cas_za_mesic += time2sec(seznam_mesicu[i][-1]) - time2sec(seznam_mesicu[i][1])
    else:
        print(f"{mesic}/{rok}: {sec2time(cas_za_mesic)}")
        mesic = seznam_mesicu[i][0].split(".")[1]
        rok = seznam_mesicu[i][0].split(".")[2]
        cas_za_mesic = time2sec(seznam_mesicu[i][-1]) - time2sec(seznam_mesicu[i][1])
print(f"{mesic}/{rok}: {sec2time(cas_za_mesic)}")
